...
溧水一方「溧水历史」分站的站点库。内容在 `lishui-history`，共享底座在 `lishui-kit`，本站只写历史分站特有的部分。

- 域名：`lishi.lishui.org`（中文在根路径，英文在 `/en/` 下）
- 生成器：Astro 5（静态输出，产物是纯 HTML）
- 规划依据：《溧水一方建设规划 v1.3》《溧水历史分站计划 v1.0》，技术选型见《技术选型详解》
...
2. **分类与分期**：六类内容（`CATEGORIES`）与五期分期（`PERIODS`）是历史分站特有的，属站点层；kit 不认这套分类，只提供 `catLabel` 这类通用助手。
